...
This one is pretty straightforward, vanilla HTML.

On the first version, you just `document.createElement('div')` and then inject it into the root div container that you should create as an HTML right after the <body> tag. 

On the extra step, you need to remove the root HTML definition done by hand and

1. define the root element with the `document.createElement` syntax
2. perform all the same steps for the new div with `Hello world`
3. inject the container into the root, then get the body and inject the root into the body
